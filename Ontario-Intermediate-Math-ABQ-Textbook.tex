...
\begin{example}{Using a complement in probability}{probcomplement}
Two fair coins are tossed. What is the probability of getting at least one head?
\tcblower
The only outcome with no heads is two tails, with probability
\[
...
For $A(-4,6)$ and $B(8,-2)$, find the midpoint and the distance between the points.
\tcblower
\[
\text{Midpoint}=\left(\frac{-4+8}{2},\frac{6+(-2)}{2}\right)=\left(\frac{4}{2},\frac{4}{2}\right)=(2,2).
\]
\[
\text{Distance}=\sqrt{(8-(-4))^2+(-2-6)^2}=\sqrt{12^2+(-8)^2}=\sqrt{208}=4\sqrt{13}.
\]
\answerline Midpoint $(2,2)$ and distance $4\sqrt{13}$
\end{example}
...
\begin{enumerate}
  \item $(3,7)$
  \item $d=\sqrt{(8-2)^2+(10-1)^2}=\sqrt{117}=3\sqrt{13}$
  \item $x^2+y^2=25$
  \item Equal slopes
\end{enumerate}
...
\begin{verbatim}
latexmk -pdf Ontario-Intermediate-Math-ABQ-Textbook.tex
\end{verbatim}
If you prefer \texttt{pdflatex}, run it repeatedly until the table of contents and cross-references stop changing.
